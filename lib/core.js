...
const { sanitizeText } = require('./utils/textHelpers');
...
class ThermalPrinter {
  constructor(initConfig) {
    const initialConfig = { width: 48, ...initConfig };

    const getInterface = require('./interfaces');

    this.Interface = getInterface(initialConfig.interface, initialConfig.options, initialConfig.driver);
    this.buffer = null;
    this.config = null;
    this.printer = null;
    ...
    this.config = {
      ...
      characterSet: initialConfig.characterSet || 'SLOVENIA',
      removeSpecialCharacters: initialConfig.removeSpecialCharacters || false,
      lineCharacter: initialConfig.lineCharacter || '-',
  ...
  setTextNormal() {
    ...
    this.config.width = this.config.initialConfig.width;
  }
  ...
  setTextDoubleWidth() {
    ...
    // Not certain about this, test when you use it
    this.config.width = this.config.initialConfig.width / 2;
  }
  ...
  /**
   * @description Goes from 0 to 7
   * @param {number} width
   * @param {number} height
   */
  setTextSize(width, height) {
    this.append(this.printer.setTextSize(width, height));
    this.config.width = this.config.initialConfig.width / (width + 1);
  }
  ...
  table(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      const text = sanitizeText(data[i].toString());
      this.append(text);

      const spaces = cellWidth - text.length;
  ...
  tableCustom(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      let tooLong = false;
      const obj = data[i];
      obj.text = sanitizeText(obj.text.toString());
  ...
  append(text) {
    if (typeof text === 'string') {
      ...
      if (this.printer.config.TXT_MARGIN_LEFT_SPACE) {
        text = ' '.repeat(this.printer.config.TXT_MARGIN_LEFT_SPACE) + text;
      }
    ...
    // Append buffer
    if (text) {
      let bufferArrayToConcat = [text];

      if (this.buffer) {
        bufferArrayToConcat.unshift(this.buffer);
        this.printer.config.TXT_MARGIN_LEFT && bufferArrayToConcat.unshift(this.printer.config.TXT_MARGIN_LEFT);

        this.buffer = Buffer.concat(bufferArrayToConcat);
      } else {
        this.printer.config.TXT_MARGIN_LEFT && bufferArrayToConcat.unshift(this.printer.config.TXT_MARGIN_LEFT);
